/*
    Initial author: (https://github.com/)Convery for Ayria.se
    License: MIT
    Started: 2016-10-8
    Notes:
        This is a wrapper around Jamis Hoo's brilliant implementation.
        See his work at: https://github.com/JamisHoo
*/

#include "SMS4.h"
#include <memory>

#pragma region Thirdparty
/******************************************************************************
*  Copyright (c) 2015 Jamis Hoo
*  Distributed under the MIT license 
*  (See accompanying file LICENSE or copy at http://opensource.org/licenses/MIT)
*  
*  Project: 
*  Filename: sm4_cbc.cc 
*  Version: 1.0
*  Author: Jamis Hoo
*  E-mail: hoojamis@gmail.com
*  Date: May 16, 2015
*  Time: 17:01:33
*  Description: SM4(128 bit) Cipher Block Chaining Mode(CBC) 
*****************************************************************************/
inline uint32_t left_rotate(const uint32_t x, const size_t i)
{
    return x << i | (x >> (sizeof(uint32_t) * 8 - i));
}
inline uint32_t tauTransformation(const uint32_t x)
{
    constexpr uint8_t Sbox[256] =
    {
        
    };
    uint32_t val;
    val = Sbox[x >> 0 & 0xff] << 0 | Sbox[x >> 8 & 0xff] << 8 |
        Sbox[x >> 16 & 0xff] << 16 | Sbox[x >> 24 & 0xff] << 24;
    return val;
}
inline uint32_t endianConvert(const uint32_t x)
{
    return (x << 24 & 0xff000000) | (x << 8 & 0x00ff0000) |
        (x >> 8 & 0x0000ff00) | (x >> 24 & 0x000000ff);
}
inline uint32_t LTransformation(const uint32_t x)
{
    return x ^ left_rotate(x, 2) ^ left_rotate(x, 10) ^
        left_rotate(x, 18) ^ left_rotate(x, 24);
}
inline uint32_t L1Transformation(const uint32_t x)
{
    return x ^ left_rotate(x, 13) ^ left_rotate(x, 23);
}
inline uint32_t TTransformation(const uint32_t x)
{
    return LTransformation(tauTransformation(x));
}
inline uint32_t FFunction(const uint32_t x[], const uint32_t rkey)
{
    return x[0] ^ TTransformation(x[1] ^ x[2] ^ x[3] ^ rkey);
}
void keyExpansion(const uint8_t key[], uint32_t keys[])
{
    uint32_t mk[4];
    mk[0] = key[0] << 24 | key[1] << 16 | key[2] << 8 | key[3] << 0;
    mk[1] = key[4] << 24 | key[5] << 16 | key[6] << 8 | key[7] << 0;
    mk[2] = key[8] << 24 | key[9] << 16 | key[10] << 8 | key[11] << 0;
    mk[3] = key[12] << 24 | key[13] << 16 | key[14] << 8 | key[15] << 0;
    constexpr uint32_t FK[4] =
    {
        0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC
    };
    constexpr uint32_t CK[32] =
    {
        0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
        0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
        0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
        0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
        0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
        0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
        0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
        0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
    };
    uint32_t k[36];
    k[0] = mk[0] ^ FK[0], k[1] = mk[1] ^ FK[1],
        k[2] = mk[2] ^ FK[2], k[3] = mk[3] ^ FK[3];

    for (size_t i = 0; i < 32; ++i)
        keys[i] = k[i + 4] = k[i] ^ L1Transformation(tauTransformation(k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ CK[i]));
}
void sm4Iteration(const uint32_t keys[], const uint32_t plain[], uint32_t cipher[])
{
    uint32_t x[36];
    x[0] = endianConvert(plain[0]);
    x[1] = endianConvert(plain[1]);
    x[2] = endianConvert(plain[2]);
    x[3] = endianConvert(plain[3]);

    for (size_t i = 0; i < 32; ++i)
        x[i + 4] = FFunction(x + i, keys[i]);

    cipher[0] = endianConvert(x[35]);
    cipher[1] = endianConvert(x[34]);
    cipher[2] = endianConvert(x[33]);
    cipher[3] = endianConvert(x[32]);
}
#pragma endregion

static void CBCEncrypt(const char *Plaintext, size_t Length, const char *Key, const char *IV, char *Ciphertext)
{
    uint32_t Keys[32]; 
    keyExpansion((const uint8_t*)(Key), Keys);

    char LocalIV[16];
    std::memcpy(LocalIV, IV, 16);

    while(Length)
    {
        for (int i = 0; i < 16; ++i)
            Ciphertext[i] = (Plaintext[i] ^ LocalIV[i]);

        sm4Iteration(Keys, (uint32_t *)Ciphertext, (uint32_t *)Ciphertext);
        std::memcpy(LocalIV, Ciphertext, 16);

        Plaintext += 16;
        Ciphertext += 16;
        Length -= 16;
    }
}
static void CBCDecrypt(const char *Ciphertext, size_t Length, const char *Key, const char *IV, char *Plaintext)
{
    uint32_t Keys[32]; 
    keyExpansion((const uint8_t*)(Key), Keys);

    for (int i = 0; i < 16; ++i)
    {
        Keys[i] ^= Keys[31 - i];
        Keys[31 - i] ^= Keys[i];
        Keys[i] ^= Keys[31 - i];
    }

    char Temp[16];
    char LocalIV[16];
    std::memcpy(LocalIV, IV, 16);

    while(Length)
    {
        std::memcpy(Temp, Ciphertext, 16);
        sm4Iteration(Keys, (uint32_t *)Ciphertext, (uint32_t *)Plaintext);

        for (int i = 0; i < 16; ++i)
            Plaintext[i] = (Plaintext[i] ^ LocalIV[i]);

        std::memcpy(LocalIV, Temp, 16);

        Plaintext += 16;
        Ciphertext += 16;
        Length -= 16;
    }
}

std::string COAL::SMS4::Encrypt(std::string &Key, std::string &IV, std::string Plaintext)
{
    // Pad the input if it's not a multiple of 16.
    if(Plaintext.size() % 16)
        Plaintext.append(16 - (Plaintext.size() % 16), '\0');

    // Pad the key and IV if less than 128 bits.
    if (Key.size() < 16) Key.append(16 - Key.size(), '\0');
    if (IV.size() < 16) IV.append(16 - IV.size(), '\0');

    // Create a new buffer and encrypt into it.
    auto Buffer = std::make_unique<char[]>(Plaintext.size());
    CBCEncrypt(Plaintext.data(), Plaintext.size(), Key.data(), IV.data(), Buffer.get());

    return std::string(Buffer.get(), Plaintext.size());
}
std::string COAL::SMS4::Decrypt(std::string &Key, std::string &IV, std::string Ciphertext)
{
    // Pad the input if it's not a multiple of 16, may ruin the last block.
    if(Ciphertext.size() % 16)
        Ciphertext.append(16 - (Ciphertext.size() % 16), '\0');

    // Pad the key and IV if less than 128 bits.
    if (Key.size() < 16) Key.append(16 - Key.size(), '\0');
    if (IV.size() < 16) IV.append(16 - IV.size(), '\0');

    // Create a new buffer and encrypt into it.
    auto Buffer = std::make_unique<char[]>(Ciphertext.size());
    CBCDecrypt(Ciphertext.data(), Ciphertext.size(), Key.data(), IV.data(), Buffer.get());

    return std::string(Buffer.get(), Ciphertext.size());
}
